#!/usr/bin/env bash
# astra-luna cross-platform interop test (run via bash tests/run-interop.sh)
#
# Proves both implementations share one state directory safely: the
# execution.json that astra-luna.ps1 writes is readable by astra-luna.sh and
# vice versa (flat JSON, identical canonical key set, same SHA-256 binding).
# This is the contract that makes ASTRA_LUNA_STATE_DIR portable across a
# Windows / POSIX (WSL, macOS) boundary.
#
# Scenarios (each in a disposable ASTRA_LUNA_STATE_DIR + CODEX_HOME):
...
SH="$PLUGIN_DIR/scripts/astra-luna.sh"
PS1S="$PLUGIN_DIR/scripts/astra-luna.ps1"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/astra-luna-interop-XXXXXX")"
WINTMP="$TMP"
...
export ASTRA_LUNA_STATE_DIR="$WINTMP/state"
export CODEX_HOME="$WINTMP/codex-home"
mkdir -p "$ASTRA_LUNA_STATE_DIR" "$CODEX_HOME"
...
# sandbox installs (agents + skill for web-manual staging in both impls)
...
if keys_ok "$ASTRA_LUNA_STATE_DIR/packets/$ID_A.execution.json"; then ok; fi
...
if keys_ok "$ASTRA_LUNA_STATE_DIR/packets/$ID_B.execution.json"; then ok; fi
...
if keys_ok "$ASTRA_LUNA_STATE_DIR/packets/$ID_C.execution.json"; then ok; fi
