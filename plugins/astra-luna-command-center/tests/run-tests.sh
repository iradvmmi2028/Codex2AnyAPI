#!/usr/bin/env bash
# astra-luna integration tests (POSIX: run via bash tests/run-tests.sh)
#
# Behavioural twin of tests/run-tests.ps1. Verifies the astra-luna.sh state
...
# ASTRA_LUNA_STATE_DIR are used (no writes into the plugin checkout).
#
# Coverage:
...
S="$PLUGIN_DIR/scripts/astra-luna.sh"

TMP="$(mktemp -d "${TMPDIR:-/tmp}/astra-luna-test-XXXXXX")"
export CODEX_HOME="$TMP/codex-home"
export ASTRA_LUNA_STATE_DIR="$TMP/state"
mkdir -p "$CODEX_HOME" "$ASTRA_LUNA_STATE_DIR"
...
grep -Eq 'astra-luna\.sh.* init' "$TMP/err.txt" && ok || bad "doctor prints the init fix command"

# 2. init installs
rc=$(run init); echo "init rc=$rc"
assert "init" 0 "$rc"
for a in astra-explorer.toml astra-worker.toml astra-tester.toml astra-reviewer.toml astra-researcher.toml; do
    [[ -f "$CODEX_HOME/agents/$a" ]] && ok || bad "init writes agent TOML $a"
done
[[ -f "$CODEX_HOME/skills/astra-command-center/SKILL.md" ]] && ok || bad "init installs skill"
[[ ! -f "$CODEX_HOME/config.toml" ]] && ok || bad "init never writes config.toml"

# 2b. init is idempotent and never overwrites an existing agent file
#     (coexistence contract with the old sol-luna plugin / hand edits)
printf 'name = "astra_worker_customized"  # hand-edited, must survive\n' > "$CODEX_HOME/agents/astra-worker.toml"
rc=$(run init); echo "re-init rc=$rc"
assert "re-init" 0 "$rc"
grep -q 'astra_worker_customized' "$CODEX_HOME/agents/astra-worker.toml" \
    && ok || bad "re-init preserves an existing agent TOML"
grep -q 'existing agent preserved' "$TMP/out.txt" && ok || bad "re-init reports preserved agents"
...
grep -q 'gpt-6-astra' "$TMP/out.txt" && ok || bad "doctor reports astra orchestrator model"
grep -q '"luna_effort": "max"' "$TMP/out.txt" && ok || bad "doctor reports luna max effort"

# 15. doctor exposes the codex CLI config probe (regression: doctor used to
#     report healthy while every codex exec died on a config parse error)
rc=$(run doctor); echo "doctor probe-keys rc=$rc"
grep -q '"codex_cli_config"' "$TMP/out.txt" && ok || bad "doctor reports codex_cli_config"

# 15b. broken codex config.toml -> doctor exits 2 and names the culprit (the
#      real-world incident: a nested [features.*] table breaks EVERY codex
#      exec with 'invalid type: map, expected a boolean' while the desktop
#      app keeps working). A fake codex binary via ASTRA_LUNA_CODEX_BIN (the
#      probe only passes fixed args) keeps this deterministic.
cat > "$TMP/fake-codex.sh" <<'EOF'
#!/usr/bin/env bash
if [ -f "$CODEX_HOME/config.toml" ] && grep -qE '^\[features\.' "$CODEX_HOME/config.toml"; then
    echo "Error loading configuration: config.toml: invalid type: map, expected a boolean" >&2
    exit 1
fi
...
export ASTRA_LUNA_CODEX_BIN="$TMP/fake-codex.sh"
printf 'model = "gpt-6-astra"\n[features]\nfast_mode = true\n[features.context_management]\nexperimental_mode = true\n' > "$CODEX_HOME/config.toml"
rc=$(run doctor); echo "doctor broken-config rc=$rc"
assert "doctor on broken codex config -> exit 2" 2 "$rc"
grep -q 'features\.context_management' "$TMP/err.txt" && ok || bad "doctor names the offending [features.*] table"
grep -q '"codex_cli_config": false' "$TMP/out.txt" && ok || bad "doctor verdict codex_cli_config=false"
# repaired config (all-boolean [features]) -> healthy again
printf 'model = "gpt-6-astra"\n[features]\nfast_mode = true\n' > "$CODEX_HOME/config.toml"
rc=$(run doctor); echo "doctor repaired-config rc=$rc"
assert "doctor on repaired codex config exits 0" 0 "$rc"
unset ASTRA_LUNA_CODEX_BIN
rm -f "$CODEX_HOME/config.toml"

# 15c. init refreshed the installed skill WITH the launcher runtime clone
#      (regression: the installed skill copy referenced ../../scripts/ which
#      does not exist under ~/.codex/skills)
[[ -f "$CODEX_HOME/skills/astra-command-center/scripts/astra-luna.sh" ]] && ok || bad "init clones the launcher sh into the installed skill"
[[ -f "$CODEX_HOME/skills/astra-command-center/scripts/astra-luna.ps1" ]] && ok || bad "init clones the launcher ps1 into the installed skill"
[[ -f "$CODEX_HOME/skills/astra-command-center/config/config.toml.example" ]] && ok || bad "init clones config/ into the installed skill"

# 15d. chatgpt-web transport: doctor reports the environment probe verdict
#      (offline: no python/playwright/browser needed) and the transport list
#      contains the chatgpt-web key
rc=$(run doctor); echo "doctor chatgpt-web keys rc=$rc"
grep -q '"chatgpt_web"' "$TMP/out.txt" && ok || bad "doctor reports chatgpt_web probe"
grep -q '"transports": { [^}]*"chatgpt_web"' "$TMP/out.txt" && ok || bad "transports map contains the chatgpt_web key"
# 15e. chatgpt-web dispatch on a FRESH packet with python invisible and no
#      ASTRA_LUNA_PYTHON -> exit 2 with a python-missing message (no silent
#      degradation; a reused 'done' packet would fail the state check first)
printf '{"goal":"chatgpt-web transport smoke test","scope":["none"],"acceptance":["no python needed"],"constraints":["test only"]}' > "$TMP/cw-spec.json"
rc=$(run new-packet --spec "$TMP/cw-spec.json")
ID3="$(sed -n 's/^created: //p' "$TMP/out.txt")"
[[ -n "$ID3" ]] && ok || bad "fresh packet for chatgpt-web tests"
PATH_SAVE="$PATH"
unset ASTRA_LUNA_PYTHON
export PATH="/usr/bin:/bin"
rc=$(run dispatch --id "$ID3" --transport chatgpt-web); echo "chatgpt-web no-python rc=$rc"
export PATH="$PATH_SAVE"
assert "chatgpt-web dispatch without python -> exit 2" 2 "$rc"
grep -q 'python' "$TMP/err.txt" && ok || bad "chatgpt-web no-python error names python"
# 15f. chatgpt-web dispatch with a FAKE python failing with exit 2 is reported
#      as an environment failure -> exit 2
printf '#!/usr/bin/env bash\nexit 2\n' > "$TMP/fake-py.sh"
chmod +x "$TMP/fake-py.sh"
export ASTRA_LUNA_PYTHON="$TMP/fake-py.sh"
rc=$(run dispatch --id "$ID3" --transport chatgpt-web); echo "chatgpt-web fake-py rc=$rc"
unset ASTRA_LUNA_PYTHON
assert "chatgpt-web fake-py env failure -> exit 2" 2 "$rc"
grep -q 'chatgpt-web failed' "$TMP/err.txt" && ok || bad "chatgpt-web failure message present"
...
PACKETS="$ASTRA_LUNA_STATE_DIR/packets"
grep -q '"model":"gpt-5.6-luna"' "$PACKETS/$ID.execution.json" && ok || bad "packet pins gpt-5.6-luna"
grep -q '"effort":"max"' "$PACKETS/$ID.execution.json" && ok || bad "packet pins max effort"
...
if grep -q 'WATCHDOG-ALIVE' "$ASTRA_LUNA_STATE_DIR/results/$IDW.result.md" 2>/dev/null; then ok; else bad "CPU-active worker result not captured"; fi
...
STORED_SHA="$(sha_of "$ASTRA_LUNA_STATE_DIR/results/$ID.summary.md")"
grep -q "\"summary_sha256\":\"$STORED_SHA\"" "$PACKETS/$ID.execution.json" \
    && ok || bad "summary_sha256 matches the stored copy"
